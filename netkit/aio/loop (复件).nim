...
type
  ...
  TaskCode {.pure, size: sizeof(uint8).} = enum # TODO: size 优化，与 Promise 一致
    ...
    NotifyRequest, NotifyResponse, NotifyExec
...
proc accept(s: SocketChannel, g: ptr EventLoopGroup) = 
  # TODO: 考虑 s.loop 尚未分配
  
  # TODO：loops[0] 修改为 getCurrentLoop()
  if g.loops[0].id == s.loop.id:
    ...
    let t = cast[ptr NotifyTask](allocShared0(sizeof(NotifyTask)))
    t.code = TaskCode.NotifyRequest
    t.task = task
    # t.loop = getCurrentLoop()
    var buffer: uint64 = cast[ByteAddress](t).uint64
    if s.loop.pipe[1].write(buffer.addr, sizeof(uint64)) < 0: # TODO: 考虑 errorno enter,eagain,ewouldblock,...
      raiseOSError(osLastError())
