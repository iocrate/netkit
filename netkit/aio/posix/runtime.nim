...
proc getCurrentExecutor*(): ExecutorId {.inline.} =
  currentExecutorId
...
proc exec*(id: ExecutorId, r: Runnable) =
  # TODO: check executorId bound
  if isPrimaryExecutor():
    gScheduler.executors[id].execSpsc(r)
  else:
    gScheduler.executors[id].execMpsc(r)
...
iterator executors*(id: ExecutorGroupId): ExecutorId = 
  let group = gScheduler.executorGroups[id].value.addr
  if group.cap > 0:
    for i in 0..<group.cap:
      yield ((group.start + i) mod gScheduler.mask + 1)

proc runExecutor(id: ExecutorId) {.thread.} =
...
proc `=destroy`*(r: var IoRegistry) =
  if r.owner != nil:
    if currentExecutor == r.owner:
      currentExecutor[].unregisterHandle(r.id)
    else:
      let id = r.id
      let exec = if isPrimaryExecutor(): execSpsc else: execMpsc
      r.owner[].exec proc =
        currentExecutor[].unregisterHandle(id)

proc `=`*(dest: var IoRegistry, source: IoRegistry) {.error.}
 
proc initIoRegistry*(handle: IoHandle): IoRegistry = 
  result.owner = currentExecutor
  result.id = result.owner[].registerHandle(handle.cint)
  result.handle = handle

proc registerReadable*(r: IoRegistry, p: Pollable) =
  if currentExecutor == r.owner:
    currentExecutor[].registerReadable(r.id, p)
  else:
    let id = r.id
    let exec = if isPrimaryExecutor(): execSpsc else: execMpsc
    r.owner[].exec proc =
      currentExecutor[].registerReadable(id, p)

proc unregisterReadable*(r: IoRegistry) =
  if currentExecutor == r.owner:
    currentExecutor[].unregisterReadable(r.id)
  else:
    let id = r.id
    let exec = if isPrimaryExecutor(): execSpsc else: execMpsc
    r.owner[].exec proc =
      currentExecutor[].unregisterReadable(id)

proc registerWritable*(r: IoRegistry, p: Pollable) =
  if currentExecutor == r.owner:
    currentExecutor[].registerWritable(r.id, p)
  else:
    let id = r.id
    let exec = if isPrimaryExecutor(): execSpsc else: execMpsc
    r.owner[].exec proc =
      currentExecutor[].registerWritable(id, p)

proc unregisterWritable*(r: IoRegistry) =
  if currentExecutor == r.owner:
    currentExecutor[].unregisterWritable(r.id)
  else:
    let id = r.id
    let exec = if isPrimaryExecutor(): execSpsc else: execMpsc
    r.owner[].exec proc =
      currentExecutor[].unregisterWritable(id)
